...
/**
 * Tool to set a field access watchpoint.
 * The VM will suspend when the field is read.
 */
public class WatchpointAccessTool implements ToolHandler {
    ...
    public ToolDefinition getDefinition() {
        ...
        return new ToolDefinition(
            "watchpoint_access",
            "Break when field is READ. May show PENDING if class not loaded. Use wait_for_stop after resume to catch the access.",
            schema
        );
    }
    ...
    public ToolResult execute(McpRequest request) {
        ...
        try {
            ...
            String watchpointId = wpManager.setAccessWatchpoint(className, fieldName);

            boolean isPending = wpManager.getWatchpoint(watchpointId).isPending();

            StringBuilder sb = new StringBuilder();
            if (isPending) {
                sb.append("Access watchpoint set (PENDING - class not yet loaded).\n");
                sb.append("The watchpoint will be activated when the class is loaded.\n");
            } else {
                sb.append("Access watchpoint set successfully.\n");
            }
            ...
            return ToolResult.error("Failed to set access watchpoint: " + e.getMessage());
        }
    }
}
